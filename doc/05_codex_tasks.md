...
### T208 区域/长截图右键取消 + 可配置热键 + 自动保存目录

- 区域截图与长截图统一支持鼠标右键取消（等价 ESC / 工具条 ✕）
- Overlay 吞掉右键/菜单相关事件，避免底层应用抢焦点造成取消失效
...
- 三个设置按钮职责落地：
  - `btnTopSettings`、`sideConfigButton`、`btnHotkeySetting`：打开截图设置
  - `sideBottomButton`：打开“关于/反馈”（`AboutDialog`）

## 进行中

### T201 长截图稳定性增强

- 目标：复杂动态页面下进一步降低错位与重复拼接
- 状态：进行中
...
- “独立滚动模式”已下线为主路径，当前标准路径为“区域模式 -> 长截图”。
