...
7. 实验长截图层（默认关闭）：`LongCaptureSessionController` 与流水线模块

> 说明：长截图目前采用“实验特性 + 默认关闭”的接入方式，标准截图主流程不依赖该层。
...
长截图已改为“实验特性 + 默认关闭”的独立链路，避免继续污染稳定主线。T401/T402 当前完成的是 Windows GDI 手动滚轮版实验链路接回，不代表功能已验收通过。
